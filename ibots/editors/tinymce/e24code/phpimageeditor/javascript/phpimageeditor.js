...
function close_open_save_gg(){
	if (  $("#panel_999").attr ('show_save')==1  ){  // 4 - CLOSE SAVE TAB
		$("#panel_999").fadeOut("normal");
		$("#panel_999").attr ('show_save',0);
	} else {
		$("#panel_999").css('display','block');
		$("#panel_999").fadeTo("normal", 1.0);
		$("#panel_999").attr ('show_save',1);
	}
	return;
}
// функция вызывается при нажатии на кнопку tab
...
function phpimageeditor_init()
{
   ...
   $("#menuitem_" + MenuRotate).click(function()
   {
    ...
    if ($("#menuitem_" + 999) != null)
    {
        $("#menuitem_" + 999).click(function()
        {
			close_open_save_gg();
        });
    }
    ...
    for (i = 0; i < 4; i++)		// здесь мы скрываем все табы кроме SAVE TAB, он просто не выводится (style=none) в index.php
    {
        if ($('#panel_'+i) != null)
        {
            if (i == selectedIndex)
