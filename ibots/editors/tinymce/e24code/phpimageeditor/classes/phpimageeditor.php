	...
	*/
	...
	class PHPImageEditor 
	{
		...
		function Action($actionType)
		{
			...
			{
				//print_r ($this->srcEdit); print "<br /><br />";		print_r ($_POST);			die();
				
						if (  $_POST['gg_save_to']=='org'  )	$this->SaveImage($this->resourceWorkWith, $this->srcEdit, $this->mimeType, $_POST['gg_save_prefix']);
				else 	if (  $_POST['gg_save_to']=='jpg'  )	$this->SaveImage($this->resourceWorkWith, $this->srcEdit, 'image/jpeg', $_POST['gg_save_prefix']);						
				else 	if (  $_POST['gg_save_to']=='gif'  )	$this->SaveImage($this->resourceWorkWith, $this->srcEdit, 'image/gif', $_POST['gg_save_prefix']);
				else 	if (  $_POST['gg_save_to']=='png'  )	$this->SaveImage($this->resourceWorkWith, $this->srcEdit, 'image/png', $_POST['gg_save_prefix']);
...
/*				PIE_Echo('<script language="javascript" type="text/javascript">'.$reloadParentBrowser.'window.open(\'\',\'_parent\',\'\');window.close();</script>');	*/
				PIE_Echo('<script language="javascript" type="text/javascript">window.open("/ibots/editors/tinymce/e24code/AjexFileManager/index.html?type=image&connector=php&returnTo=tinymce&lang=ru&skin=dark&contextmenu=true", "_self")</script>');
		...
		function SaveImage($image, $toSrc, $mimeType = -1, $ggprefix="")
		{
			if ($mimeType == -1)
				$mimeType = $this->mimeType;
			if (  $ggprefix!=''  ){
				$path_parts = pathinfo($toSrc);
				$toSrc = $path_parts['dirname']."/".str_replace('.'.$path_parts['extension'], "", $path_parts['basename']).'_'.$ggprefix.'.'.$path_parts['extension'];
				//print $toSrc ; // die();
			}
			
			if ($mimeType == image_type_to_mime_type(IMAGETYPE_JPEG))
			{	
				if (  isset($_POST['gg_quality'])  )	imagejpeg($image, $toSrc, $_POST['gg_quality']);
				else imagejpeg($image, $toSrc);		
			}
			else if ($mimeType == image_type_to_mime_type(IMAGETYPE_GIF))
			{	
					...
					if($file != '.' && $file != '..') 
					{
						...
						if (stripos($srcfile, ".svn") === false)
						{
							if(is_file($srcfile)) 
							{
								...
								if (  (substr_count($srcfileLower, ".jpg") == 0 && substr_count($srcfileLower, ".gif") == 0 && substr_count($srcfileLower, ".png") == 0))
									$doDelete = false; 
