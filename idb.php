...
class database {
	...
	*/
	function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
		...
		} else {		// ggtr ($host); ggtr ($user); ggtr ($pass); ggd();
			if (!($this->_resource = mysql_connect( $host, $user, $pass, true ))) {
				if (!($this->_resource = @mysql_connect( "localhost", "anonymous", "", true ))) {
					$mosSystemError = 2;
					if ($goOffline) {
						$basePath = dirname( __FILE__ );
						print "we go offline";
...
//		if (  $this->debug==1  ) ggtr (pofigu($sql));
		//$this->_sql = $this->replacePrefix( pofigu($sql), $prefix );
		$this->_sql = $this->replacePrefix( $sql, $prefix );
//		if (  $this->debug==1  ) ggtr2 ($this);
//		if (  $this->debug==1  ) ggd ($this->_sql,4);
	...
	*/
	function query() {
		...
		if ($this->_debug) {
			$this->_ticker++;
	  		$this->_log[] = $this->_sql;
                        $fh = fopen(site_path.'/0_sql.html', "a");
                        fwrite(  $fh, ( num::fillzerro($this->_ticker,7).' '.$this->_sql ).'<br>'  );
                        fclose($fh);
		...
		if (!$this->_cursor) {
			...
			// if ($this->_debug) {
                        if (  1  ) {
                                $fh = fopen(site_path.'/_sql.log', "a");
                                fwrite(  $fh, ( chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).num::fillzerro($this->_ticker,7).' ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ' ).chr(13).chr(10)  );
                                fwrite(  $fh, ( ' '.$this->_sql ).chr(13).chr(10)  );
                                fwrite(  $fh, ( ' '.mysql_error( $this->_resource ) ).chr(13).chr(10)  );
				trigger_error( mysql_error( $this->_resource ), E_USER_NOTICE );
                                //print mysql_error( $this->_resource );
				echo "<pre>" . $this->_sql . "</pre>\n";
				if (function_exists( 'debug_backtrace' )) {
					foreach( debug_backtrace() as $back) {
						if (@$back['file']) {
							echo '<br />'.$back['file'].':'.$back['line'];
                                                        fwrite(  $fh, ( ' '.$back['file'].':'.$back['line'] ).chr(13).chr(10)  );
						}
					}
				}
                                fwrite(  $fh, ( num::fillzerro($this->_ticker,7).' ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ERROR ' ).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10).chr(13).chr(10)  );
                                fclose($fh);
			}
			return false;
		}
	...
	*/
	function updateOrder( $where='' ) {
		...
		if (!array_key_exists( 'ordering', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНЕ: ".strtolower(get_class( $this ))." не поддерживает переупорядочивание";
			return false;
		}
	...
	*/
	...
	function checkout( $user_id, $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНИЕ: ".strtolower(get_class( $this ))." не поддерживает снятие блокировок.";
			return false;
		}
		$k = $this->_tbl_key;
		if ($oid !== null) {
	...
	function checkin( $oid=null ) {
		if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
			$this->_error = "ВНИМАНИЕ: ".strtolower(get_class( $this ))." не поддерживает блокировки.";
			return false;
		}
		$k = $this->_tbl_key;
		if ($oid !== null) {
