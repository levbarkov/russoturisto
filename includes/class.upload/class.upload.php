...
class upload {
        ...
        if ($this->uploaded) {
            ...
            if ($this->file_is_image) {
                if ($h = fopen($this->file_src_pathname, 'r')) {
                    ...
                    $info = getimagesize($this->file_src_pathname);
                    if (is_array($info)) {
                        $this->image_src_x    = abs($info[0]);
                        $this->image_src_y    = abs($info[1]);
    ...
    function imagecreatefrombmp($filename) {
        ...
        $P = 0;
        $Y = ($is_reversed) ? 0 : $bmp['height']-1;
        while (($is_reversed && $Y < $bmp['height']) || (!$is_reversed && $Y >= 0)) {
            $X=0;
            while ($X < $bmp['width']) {
