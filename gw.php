...
$user_agents = array(
    ...
    'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.1.5pre) Gecko/20091027 Ubuntu/8.10 (intrepid) Shiretoko/3.5.5pre',
...
function get_useragent () {
    global $user_agents;
    $i = array_rand($user_agents, 1);
    return $user_agents[$i];
}

function request_page ($url, $params='', $method='get', $cookie=null) {
    ...
    if (function_exists('curl_init')) {
        ...
        curl_setopt($request, CURLOPT_USERAGENT, get_useragent());
        ...
        if ($method == 'post') {
            ...
            curl_setopt($request, CURLOPT_HTTPHEADER, array("Content-Type: text/html"));
        }
    ...
    elseif (function_exists('file_get_contents') && ini_get('allow_url_fopen') != 0)
...
$url = (isset($qry->url)) ? urldecode($qry->url) : null;
$params = (isset($qry->params)) ? urldecode($qry->params) : '';
$method = (isset($qry->method) && $qry->method != 'get') ? 'post' : 'get';
$cookie = (isset($qry->cookie)) ? urldecode($qry->cookie) : null;

print request_page($url, $params, $method, $cookie);
