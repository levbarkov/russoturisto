...
define( 'COM_IMAGE_BASE', site_path . "/" . 'images' );
...
function editSection( $uid=0, $scope='', $option ) {
	...
	if ($row->isCheckedOut( $my->id )) {
		$msg = 'Рубрика -'. $row->title .'- в настоящее время редактируется другим администратором';
		mosRedirect( 'index2.php?ca='. $option .'&scope='. $row->scope .'&mosmsg='. $msg );
	}
			...
			for( $i = 0; $i < $count; $i++ ) {
				switch ( $menus[$i]->type ) {
					case 'content_section':
						$menus[$i]->type = 'Таблица рубрик';
						break;

					case 'content_blog_section':
						$menus[$i]->type = 'Блог рубрик';
						break;

					case 'content_archive_section':
						$menus[$i]->type = 'Блог архива рубрик';
						break;
				}
			}
		} else {
	...
	$types[] = mosHTML::makeOption( '', 'Тип рубрики' );
	$types[] = mosHTML::makeOption( 'content_section', 'Список рубрик' );
	$types[] = mosHTML::makeOption( 'content_blog_section', 'Блог рубрик' );
	$types[] = mosHTML::makeOption( 'content_archive_section', 'Блог архива рубрик' );
...
function saveSection( $option, $scope, $task ) {
	...
	$row = new mosSection( $database );
	if (!$row->bind( $_POST )) {
		echo "<script> alert('".$row->getError()."'); document.location.href='index2.php?ca=$option&scope=$scope&task=new'; </script>\n";
		exit();
	}
	if (!$row->check()) {
		echo "<script> alert('".$row->getError()."'); document.location.href='index2.php?ca=$option&scope=$scope&task=new'; </script>\n";
	...
	switch ( $task ) {
		...
		case 'apply':
			$msg = 'Изменения рубрики сохранены';
			mosRedirect( 'index2.php?ca='. $option .'&scope='. $scope .'&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Рубрика сохранена';
			mosRedirect( 'index2.php?ca='. $option .'&scope='. $scope, $msg );
			break;
	}
}
/**
...
function removeSections( $cid, $scope, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Выберите рубрику для удаления'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( ', ', $err );
		$msg = 'Рубрики: '. $cids .' не могут быть удалены, т.к. содержат подрубрики';
		mosRedirect( 'index2.php?ca='. $option .'&scope='. $scope, $msg );
	}
	...
	$names = implode( ', ', $name );
	$msg = 'Рубрика(и): '. $names .' успешно удалена(ы)';
	mosRedirect( 'index2.php?ca='. $option .'&scope='. $scope, $msg );
}
...
function publishSections( $scope, $cid=null, $publish=1, $option ) {
	...
	if ( !is_array( $cid ) || count( $cid ) < 1 ) {
		...
		echo "<script> alert('Выберите рубрику для $action'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( $publish ) {
		if ( !$count ){
			echo "<script> alert('Невозможно опубликовать пустую рубрику $count'); window.history.go(-1);</script>\n";
			return;
		}
	}
	...
	mosRedirect( 'index2.php?ca='. $option .'&scope='. $scope );
}
...
function copySectionSave( $sectionid ) {
	...
	$msg = 'Выбранное содержимое рубрики было скопировано в рубрику '. $title;
	mosRedirect( 'index2.php?ca=sections&scope=content&mosmsg='. $msg );
}
...
function accessMenu( $uid, $access, $option ) {
	...
	mosRedirect( 'index2.php?ca='. $option .'&scope='. $row->scope );
}
...
function menuLink( $id ) {
	...
	switch ( $type ) {
		case 'content_section':
			$link 		= 'index.php?ca=content&task=section&id='. $id;
			$menutype	= 'Таблица рубрик';
			break;

		case 'content_blog_section':
			$link 		= 'index.php?ca=content&task=blogsection&id='. $id;
			$menutype	= 'Блог рубрик';
			break;

		case 'content_archive_section':
			$link 		= 'index.php?ca=content&task=archivesection&id='. $id;
			$menutype	= 'Блог архива рубрик';
			break;
	}
	...
	mosRedirect( 'index2.php?ca=sections&scope=content&task=editA&hidemainmenu=1&id='. $id,  $msg );
}
...
function saveOrder( &$cid ) {
	...
	mosRedirect( 'index2.php?ca=sections&scope=content', $msg );
} // saveOrder
